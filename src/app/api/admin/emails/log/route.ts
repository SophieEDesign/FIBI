...
export async function GET(request: NextRequest) {
  try {
    ...
    const applyFilters = <T extends { eq: (col: string, val: string) => T }>(query: T): T => {
      let next = query
      if (templateSlug) next = next.eq('template_slug', templateSlug)
      if (statusFilter === 'sent' || statusFilter === 'failed') next = next.eq('status', statusFilter)
      return next
    }

    // Prefer error_detail (migration 054); fall back if the column is not on this DB yet
    let logs: EmailLogRow[] | null = null
    let total: number | null = null

    const withDetail = await applyFilters(
      admin
        .from('email_logs')
        .select(
    ...
    if (
      withDetail.error &&
      typeof withDetail.error.message === 'string' &&
      withDetail.error.message.includes('error_detail')
    ) {
      const withoutDetail = await applyFilters(
        admin
          .from('email_logs')
          .select(
      ...
      if (withoutDetail.error) {
        console.error('Error fetching email log:', withoutDetail.error)
        return NextResponse.json({ error: 'Failed to fetch log' }, { status: 500 })
      }
      logs = (withoutDetail.data ?? []) as EmailLogRow[]
      total = withoutDetail.count
    } else if (withDetail.error) {
      console.error('Error fetching email log:', withDetail.error)
      return NextResponse.json({ error: 'Failed to fetch log' }, { status: 500 })
    } else {
      logs = (withDetail.data ?? []) as EmailLogRow[]
      total = withDetail.count
    }
